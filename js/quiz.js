...
function matchFormations() {
  ...
  const scored = formations.map(f => {
    let score = 0;
    if (ville   && f.villes.includes(ville))           score += 3;
    if (niveau  && f.niveaux_entree.includes(niveau))  score += 3;
    if (domaine && f.domaines.includes(domaine))       score += 2;
    return { ...f, score };
  });

  scored.sort((a, b) => b.score - a.score);
  const good = scored.filter(f => f.score > 0);
  return (good.length ? good : scored).slice(0, 3);
}
